...
export const applyBoundForce = ({
  bound,
  edge,
  nativePosition,
  friction,
  velocityX,
}: BoundForceProps): number => {
...
const Dragger = ({
  ...
  const mouseCursorStyle =
    ...
    setCursorStyles && isDraggingStyle
      ? "grabbing"
      : setCursorStyles
        ? "grab"
        : undefined;
  ...
  useEffect(() => {
    ...
    const observer = new Ro((entries: any[]) => {
      ...
      const { id } = entries[0].target.dataset;
  ...
  function setPosition(position: number, instant: boolean) {
    ...
    if (instant) {
      ...
      if (innerEl.current) {
        ...
        innerEl.current.style.transform = `translate3d(${roundNum(
          finalPosition
        )}px,0,0)`;
  ...
  function updateLoop(optionalFinalPosition: null | number) {
    ...
    if (optionalFinalPosition !== null) {
      ...
      velocityX.current += force; // apply force
    } else {
    ...
    const isInfinitesimal: boolean =
      roundNum(Math.abs(velocityX.current)) < 0.001;
  ...
  function onRelease(e: any) {
   ...
   const moveVector = Math.abs(downX.current - e.pageX);
    if (moveVector < 50 && onStaticClick) {
  ...
  return (
    <div
      ...
      ref={outerEl}
      className={[disabled ? " is-disabled" : null, className].join(" ")}
